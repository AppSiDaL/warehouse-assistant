...
const getMainChartOptions = () => {
	...
	return {
		chart: {
			...
			foreColor: (mainChartColors as any).labelColor,
			toolbar: {
				show: false,
			},
		},
		fill: {
			...
			gradient: {
				enabled: true,
				opacityFrom: (mainChartColors as any).opacityFrom,
				opacityTo: (mainChartColors as any).opacityTo,
			},
		},
		dataLabels: {
			enabled: false,
		...
		grid: {
			show: true,
			borderColor: (mainChartColors as any).borderColor,
			strokeDashArray: 1,
			padding: {
				left: 35,
				bottom: 15,
		...
		xaxis: {
			...
			labels: {
				style: {
					colors: [(mainChartColors as any).labelColor],
					fontSize: '14px',
					fontWeight: 500,
				},
			},
			axisBorder: {
				color: (mainChartColors as any).borderColor,
			},
			axisTicks: {
				color: (mainChartColors as any).borderColor,
			},
	...
	return {
		...
		plotOptions: {
			bar: {
				...
				borderRadius: 3,
				colors: {
					backgroundBarColors: (signupsChartColors as any).backgroundBarColors,
...
const getTrafficChannelsChartOptions = () => {
	...
	return {
		...
		stroke: {
			colors: [(trafficChannelsChartColors as any).strokeColor],
		},
		states: {
			hover: {
				filter: {
		...
		tooltip: {
			...
			x: {
				show: true,
				formatter(_:any, { seriesIndex, w }: { seriesIndex: number, w: any }) {
